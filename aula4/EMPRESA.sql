...
-- Quais projetos o Fernando trabalha?
SELECT *
FROM FUNCIONARIO AS F
...
-- Todos os trabalhaores menos o fernando
SELECT F.Pnome, T.Pnr
FROM TRABALHA_EM AS T
JOIN FUNCIONARIO AS F ON F.Cpf = T.Fcpf
WHERE F.Pnome <> 'Fernando'
  AND T.Pnr IN (
      SELECT TE.Pnr
      ...
      WHERE TE.Fcpf = (
          SELECT FF.Cpf
          FROM FUNCIONARIO AS FF
          WHERE FF.Pnome = 'Fernando'
      )
  );
...
--INNER JOIN
--Funcionários que trabalham no departamento de pesquisa 
...
-- Nome dos funcionários que estão desenvolvendo o "Produto X"
...
-- LEFT JOIN
-- Último nome dos funcionários e seus respectivos departamentos se houver
SELECT F.Unome, D.Dnome
FROM FUNCIONARIO AS F
LEFT JOIN DEPARTAMENTO AS D
ON F.Dnr = D.Dnumero

-- Encontre os departamentos que não possuem funcionarios
SELECT *
FROM DEPARTAMENTO AS D
...
-- CROSS JOIN / FULL JOIN (mesma coisa)
-- Todas as relações entre funcionario e departamento
SELECT * 
FROM FUNCIONARIO AS F
FULL JOIN DEPARTAMENTO AS D
ON F.Dnr = D.Dnumero 

-- Mostre os funcionários sem departamento e os departamentos que não possuem funcionarios
SELECT * 
FROM FUNCIONARIO AS F
FULL JOIN DEPARTAMENTO AS D
ON F.Dnr = D.Dnumero
...
-- SELF JOIN
-- Mostre apenas os funcionários que possuem supervisor
SELECT F.Pnome AS 'FUNCIONARIO', FUNFA.Unome AS 'SUPERVISOR'
FROM FUNCIONARIO AS F
JOIN FUNCIONARIO AS FUNFA
ON F.Cpf_supervisor = FUNFA.Cpf
ORDER BY SUPERVISOR

-- UNION/INTERSECT/EXCEPT
-- UNION
-- Listar todas as pessoas do banco com nome, sexo e data de nascimento
SELECT F.Pnome AS 'NOME', F.Sexo AS ' SEXO', F.Datanasc 'DATA DE NASCIMENTO'
FROM FUNCIONARIO AS F

UNION
 
SELECT D.Nome_dependente AS 'NOME', D.Sexo AS 'SEXO', D.Datanasc AS 'DATA DE NASCIMENTO'
